...
# if...elif...else csv_data：根据用户输入的数字，判断数字是正数，还是负数，还是0
num = int(input("请输入数字："))
...
# csv_data：根据输入的用户名和密码进行系统登录 -- admin/666888  root/547527  zhangsan/123456
...
"""
csv_data：三角形类型判断：根据输入的三个边的边长(正整数)，判定是等边三角形、等腰三角形、普通三角形，还是不能构成三角形。
